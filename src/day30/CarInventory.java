...
//print of the location Honda Civic in this Array
        System.out.println("countChevrolet= " + countChevrolet);
            System.out.println("countcivic= " + countcivic);

 ////print of the location Honda Civic in this Array
           for (int j = 0; j < cars.length; j++) {
  if  (cars[j].equals("Honda-Civic")) {
      System.out.println("countain Honda-civic loation " + j);

  }
    }//Splited method use to make and brand seperated
    for(String eachCar:cars) {
        if (eachCar.startsWith("Honda")) {
            String[] eachCarSplited = eachCar.split("-");
            System.out.println("Car make is " + eachCarSplited[0] + " " + "car make is " + eachCarSplited[1]);
        }}
        System.out.println("Cars with model has only 2 characters");
    int cntOf2CharModel=0;
    for(String eachCar:cars){
        String []eachCarsSplited=eachCar.split("-");
        String model=eachCarsSplited[1];
        if(eachCarsSplited[1].length()==2){
    if(model.length()==2){
        cntOf2CharModel++;
